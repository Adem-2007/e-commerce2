...
import { Lock, User, AtSign, AlertCircle, UserPlus } from 'lucide-react';
import AuthInput from './AuthInput';
import ButtonLoader from './ButtonLoader'; // <-- Import the new loader

const RegisterForm = ({ formData, handleInputChange, handleSubmit, isSubmitting, isThrottled, t, error }) => {
    return (
        <>
            {/* ... (Header and error message) ... */}
             <div className="text-left rtl:text-right mb-10">
                <h2 className="text-3xl font-semibold text-gray-800 mb-2">{t('auth_page.create_account_title')}</h2>
                <p className="text-base text-gray-500">{t('auth_page.register_subtitle')}</p>
            </div>
            ...
            <form onSubmit={handleSubmit}>
                <div className="flex flex-col gap-5">
                    <AuthInput id="name" name="name" type="text" placeholder="John Doe" value={formData.name} onChange={handleInputChange} label={t('auth_page.placeholders.name')} Icon={User} required />
                    ...
                    <button type="submit" className="flex items-center justify-center w-full bg-blue-600 text-white py-4 rounded-lg text-base font-medium transition hover:bg-blue-700 active:scale-[0.98] disabled:bg-gray-400 disabled:cursor-not-allowed mt-2" disabled={isThrottled || isSubmitting}>
                         {isSubmitting ? (
                            <ButtonLoader />
                        ) : (
                            <>
                                <UserPlus className="mr-2 rtl:mr-0 rtl:ml-2 h-5 w-5" />
                                {t('auth_page.buttons.create_account')}
                            </>
                        )}
                    </button>
                </div>
